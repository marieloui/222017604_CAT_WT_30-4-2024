...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
    $gms = $connection->prepare("INSERT INTO patient (FirstName, LastName, Gender, PhoneNumber) VALUES (?, ?, ?, ?)");
    $gms->bind_param("ssss", $FirstName, $LastName, $Gender, $PhoneNumber);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of medical record</title>
    <style>
        table {
            width: 100%;
...
<table>
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $PatientID = $row["PatientID"]; // Added this line to fetch PatientID
            echo "<tr>
                <td>" . $row["PatientID"] . "</td>
                <td>" . $row["FirstName"] . "</td>
                <td>" . $row["LastName"] . "</td>
                ...
                <td><a style='padding:4px' href='delete_patient.php?PatientID=$PatientID'>Delete</a></td> 
                <td><a style='padding:4px' href='update_patient.php?PatientID=$PatientID'>Update</a></td> 
            </tr>";
        }
    } else {
